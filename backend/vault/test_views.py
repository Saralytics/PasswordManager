...
class StoredPasswordTests(APITestCase):
    ...
    def test_create_stored_password_success(self):
        ...
        self.assertEqual(
            StoredPassword.objects.get().password, data['password'])
...
class PasswordRetrievalTests(APITestCase):
    def setUp(self):
        ...
        StoredPassword.objects.create(
            user=self.user, website=self.test_website, username='exampleuser', password=self.test_password)
    ...
    def test_retrieve_password_success(self):
        ...
        response = self.client.post(
            self.retrieve_url, {'website': self.test_website}, format='json')
    ...
    def test_retrieve_password_not_found(self):
        ...
        response = self.client.post(
            self.retrieve_url, {'website': 'https://nonexistent.com'}, format='json')
...
class PasswordUpdateTests(APITestCase):
    ...
    def test_update_password_success(self):
        ...
        response = self.client.put(self.update_url, {
                                   'website': self.test_website, 'password': 'updated123'}, format='json')
        ...
        updated_row = StoredPassword.objects.get(
            user=self.user, website=self.test_website)
    ...
    def test_update_password_missing_fields(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.put(self.update_url, {
                                   'website': self.test_website}, format='json')  # Missing 'password'
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_update_password_nonexistent(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.put(self.update_url, {
                                   'website': 'https://doesnotexist.com', 'password': 'newpass'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class PasswordDeleteTests(APITestCase):
    ...
    def test_delete_password_success(self):
        ...
        response = self.client.delete(
            self.delete_url, {'website': self.test_website}, format='json')
    ...
    def test_delete_nonexistent_password(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(
            self.delete_url, {'website': 'https://doesnotexist.com'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
